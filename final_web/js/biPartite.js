!function(){
	...
	function drawPart(data, id, p){
		...
		mainbar.append("rect").attr("class","mainrect")
			.attr("x", start).attr("y",function(d){ return d.middle-d.height/2; })
		...
		d3.select("#"+id).select(".part"+p).select(".subbars")
			...
			.attr("x", start).attr("y",function(d){ return d.y})
	...
	function edgePolygon(d){
		return [start, d.y1, bb+start, d.y2, bb+start, d.y2+d.h2, start, d.y1+d.h1].join(" ");
	}	
			...
			[0,1].forEach(function(p){			
				d3.select("#"+biP.id)
					...
					.on("click",function(d, i){
						if (p == 0) return bP.deSelectSegment(data, p, i);
						else return bP.selectSegment(data, p, i); })
